...
public class PrestitoRestituzioneService {
    ...
    // Dichiarazione dell'Enum interna alla classe
    ...
    public void restituisciLibroRandom() {
        ...
        try {
            ...
            // Creazione Header utilizzando l'Enum interno
            Message<String> messaggio = MessageBuilder
                    .withPayload(payload)
                    .setHeader(KafkaHeaders.TOPIC, "Prestito")
                    .setHeader("TIPO_OPERAZIONE", TipoOperazione.RESTITUZIONE.name()) // <-- Uso dell'Enum qui
                    .build();

            kafkaTemplate.send(messaggio);
